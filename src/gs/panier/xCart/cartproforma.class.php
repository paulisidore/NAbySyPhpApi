 ...
 */
  ...
  */
  function __construct($Bou=null,$IdSession=1,$IsTemp=null){
	...
	if (isset($IsTemp)){
		$IdSession .='temp' ;
		unset($_SESSION['panierproforma'.$IdSession]) ;
		$this->Existe=false ;
	}
	$this->Id=$IdSession;
	
	if (!$this->Existe){		
		$this->PanierId='panierproforma'.$this->Id ;
  ...
  */
  public function initCart(){
	$panierid='panierproforma'.$this->Id ;
  ...
  */
  public function GetListePdtHTML($ParametreSuplementaire=null){
    ...
    if(!empty($panier)){
      ...
      foreach($panier as $P){ 
		$Modifier='<a href="proforma.php?IdPanier='.$this->Id.'&ModifierLigne='.$P['vId'].$ParamT.'">Modifier</a>';
		$Supprimer='<a href="proforma.php?IdPanier='.$this->Id.'&SupprimerLigne='.$P['vId'].$ParamT.'">Supprimer</a>';
		$Couleur='' ;
		if ($P['typev']==1){
